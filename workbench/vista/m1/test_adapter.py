...
def fixture():
    return {
        "operator": "JT",
        "operator_note_exact": "JT VISTA M1 validation 01",
...
def test_capture_boundary_and_spatial_context():
    record = capture_webxr_state(fixture(), created_at="2026-08-27T20:00:00Z")
    ...
    assert record["capture_context"]["runtime_version"] == "VISTA_M1_PRECISION_ANNOTATION_v0.2.0"
...
def test_live_mutation_does_not_change_frozen_record():
    payload = fixture()
    record = capture_webxr_state(payload, created_at="2026-08-27T20:00:00Z")
...
def main():
    ...
    print(f"PASS all {len(tests)} VISTA M1 adapter tests")
