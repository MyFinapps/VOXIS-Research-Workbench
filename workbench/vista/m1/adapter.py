"""WebXR -> canonical VOXIS FREEZE adapter for VISTA M1."""
...
def capture_webxr_state(payload, *, created_at=None):
    ...
    if len(states_raw) != len(REPRESENTATIONS):
        raise ValueError("VISTA M1 requires exactly three representation states")
    states = [
        _representation_state(raw, ref, source, index)
        for raw, (ref, source, index) in zip(states_raw, REPRESENTATIONS)
    ]
    ...
    active_features = deepcopy(payload.get("active_features") or [])
    ...
    return emit_freeze_record(
        ...
        capture_context={
            ...
            "transform_mode": payload.get("transform_mode"),
