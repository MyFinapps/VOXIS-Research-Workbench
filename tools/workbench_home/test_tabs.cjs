...
test('stopped session reuses tab but navigates once to new server',async()=>{
 const h=harness();await h.open();h.url('http://127.0.0.1:43211/#next');await h.open();
 assert.equal(h.tabs.length,1);assert.equal(h.tabs[0].navigations.length,2);
});
